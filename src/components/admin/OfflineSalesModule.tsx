  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-background text-foreground min-h-screen space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono uppercase tracking-widest text-muted-foreground">ADMIN CONTROL CENTER</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-4 bg-card border border-border rounded-xl shadow-lg shadow-black/20">
          <span className="text-xs text-muted-foreground font-mono">SALDO TOTAL PENDIENTE DE COBRO</span>
          <p className="text-2xl font-bold text-[#FF1E42] mt-1">${totalDebt.toLocaleString()} MXN</p>
        </div>
        <div className="p-4 bg-card border border-border rounded-xl shadow-lg shadow-black/20">
          <span className="text-xs text-muted-foreground font-mono">VENTAS FÍSICAS REGISTRADAS</span>
          <p className="text-2xl font-bold text-foreground mt-1">{sales.length}</p>
        </div>
        <div className="p-4 bg-card border border-border rounded-xl shadow-lg shadow-black/20">
          <span className="text-xs text-muted-foreground font-mono">ESTADO DE COBRANZA</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 p-5 bg-card border border-border rounded-xl shadow-lg shadow-black/20 space-y-4">
          ...
          <form onSubmit={handleCreateSale} className="space-y-3 text-xs">
            <div>
              <label className="block text-muted-foreground mb-1">Nombre del Cliente</label>
              <input
                type="text"
                required
                value={form.customerName}
                onChange={(e) => setForm({ ...form, customerName: e.target.value })}
                className="w-full p-2.5 bg-muted border border-border rounded text-foreground focus:border-[#FF1E42] outline-none"
                placeholder="ej. Juan Pérez"
              />
            </div>

            <div>
              <label className="block text-muted-foreground mb-1">Teléfono WhatsApp (10 dígitos)</label>
              <input
                type="tel"
                required
            ...
            <div>
              <label className="block text-muted-foreground mb-1">Detalle del Producto / Talla</label>
              <input
                type="text"
                required
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-muted-foreground mb-1">Precio Total ($)</label>
                <input
                  type="number"
                  required
              ...
              <div>
                <label className="block text-muted-foreground mb-1">Monto Pagado ($)</label>
                <input
                  type="number"
                  required
            ...
            <div>
              <label className="block text-muted-foreground mb-1">Fecha Límite de Pago</label>
              <input
                type="date"
                value={form.dueDate}
              ...
              <tbody className="divide-y divide-zinc-800/60">
                {sales.map((sale) => (
                  <tr key={sale.id} className="hover:bg-muted">
                    ...
                    <td className="py-3 text-muted-foreground max-w-[180px] truncate">{sale.itemsSummary}</td>
